...
public class TridiagonalDecompositionHouseholder_MT_DDRB
implements TridiagonalSimilarDecomposition_F64<DMatrixRBlock> {
    ...
    public DMatrixRBlock getQ(@Nullable DMatrixRBlock Q, boolean transposed) {
        ...
        for (int i = start; i >= 0; i -= this.A.blockLength) {
            ...
            subQ.row0 = i;
            if (transposed) {
                MatrixMult_MT_DDRB.multTransB(this.A.blockLength, subQ, subU, tmp);
            } else {
                MatrixMult_MT_DDRB.mult(this.A.blockLength, subU, subQ, tmp);
            }
            if (transposed) {
                MatrixMult_MT_DDRB.multPlus(this.A.blockLength, tmp, subW, subQ);
            } else {
                MatrixMult_MT_DDRB.multPlusTransA(this.A.blockLength, subW, tmp, subQ);
            }
    ...
    public boolean decompose(DMatrixRBlock orig) {
        ...
        for (int i = 0; i < N; i += this.A.blockLength) {
            ...
            TridiagonalDecompositionHouseholder_MT_DDRB.multPlusTransA(this.A.blockLength, subU, subV, subA);
            TridiagonalDecompositionHouseholder_MT_DDRB.multPlusTransA(this.A.blockLength, subV, subU, subA);
