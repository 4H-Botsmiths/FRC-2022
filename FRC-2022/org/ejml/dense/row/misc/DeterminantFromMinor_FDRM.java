...
public class DeterminantFromMinor_FDRM {
    ...
    private final float[] levelResults;
    private final int[] levelRemoved;
    private final int[] open;
    private int numOpen;
    private final FMatrixRMaj tempMat;
    ...
    public DeterminantFromMinor_FDRM(int width, int minWidth) {
    ...
    public float compute(FMatrix1Row mat) {
        ...
        while (true) {
            ...
            if ((levelIndex = this.levelIndexes[level]) == (levelWidth = this.width - level)) {
                ...
                float val = mat.get((level - 1) * this.width + this.levelRemoved[level - 1]);
            ...
            if (levelWidth == this.minWidth) {
                ...
                float subresult = mat.get(level * this.width + this.levelRemoved[level]);
                subresult *= UnrolledDeterminantFromMinor_FDRM.det(this.tempMat);
                if (levelIndex % 2 == 0) {
                    int n = level;
