...
import org.ejml.dense.block.decomposition.qr.BlockHouseHolder_MT_FDRB;
import org.ejml.interfaces.decomposition.QRDecomposition;
import org.jetbrains.annotations.Nullable;
import pabeles.concurrency.GrowArray;

public class QRDecompositionHouseholder_MT_FDRB
implements QRDecomposition<FMatrixRBlock> {
    ...
    public FMatrixRBlock getQ(@Nullable FMatrixRBlock Q, boolean compact) {
        Q = QRDecompositionHouseholder_MT_FDRB.initializeQ(Q, this.dataA.numRows, this.dataA.numCols, this.blockLength, compact);
        this.applyQ(Q, true);
        return Q;
    }
    ...
    public void applyQ(FMatrixRBlock B, boolean isIdentity) {
        ...
        for (int i = start; i >= 0; i -= this.blockLength) {
            ...
            if (!this.saveW) {
                BlockHouseHolder_MT_FDRB.computeW_Column(this.blockLength, this.Y, this.W, this.workspace, this.gammas, this.Y.col0);
            }
            BlockHouseHolder_MT_FDRB.multTransA_vecCol(this.blockLength, this.Y, subB, this.WTA);
            MatrixMult_MT_FDRB.multPlus(this.blockLength, this.W, this.WTA, subB);
        }
    }
    ...
    public void applyQTran(FMatrixRBlock B) {
        ...
        for (int i = 0; i < minDimen; i += this.blockLength) {
            ...
            MatrixMult_MT_FDRB.multTransA(this.blockLength, this.W, subB, this.WTA);
            BlockHouseHolder_MT_FDRB.multAdd_zeros(this.blockLength, this.Y, this.WTA, subB);
        }
    }
    ...
    public boolean decompose(FMatrixRBlock orig) {
        ...
        for (int j = 0; j < m; j += this.blockLength) {
            ...
            this.Y.row0 = j;
            if (!BlockHouseHolder_MT_FDRB.decomposeQR_block_col(this.blockLength, this.Y, this.gammas)) {
                return false;
            }
            this.updateA(this.A);
        }
    ...
    protected void updateA(FSubmatrixD1 A) {
        ...
        if (A.col1 > A.col0) {
            BlockHouseHolder_MT_FDRB.computeW_Column(this.blockLength, this.Y, this.W, this.workspace, this.gammas, this.Y.col0);
            MatrixMult_MT_FDRB.multTransA(this.blockLength, this.W, A, this.WTA);
            BlockHouseHolder_MT_FDRB.multAdd_zeros(this.blockLength, this.Y, this.WTA, A);
        } else if (this.saveW) {
            BlockHouseHolder_MT_FDRB.computeW_Column(this.blockLength, this.Y, this.W, this.workspace, this.gammas, this.Y.col0);
        }
    }
