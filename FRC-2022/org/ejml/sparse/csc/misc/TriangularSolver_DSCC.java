...
public class TriangularSolver_DSCC {
    ...
    public static void solveTran(DMatrixSparseCSC G, boolean lower, DMatrixSparseCSC B, DMatrixSparseCSC X, @Nullable int[] pinv, @Nullable DGrowArray g_x, @Nullable IGrowArray g_xi, @Nullable IGrowArray g_w) {
        ...
        for (int colB = 0; colB < B.numCols; ++colB) {
            ...
            if (lower) {
                for (col = G.numRows - 1; col >= 0; --col) {
                    X_nz_count = TriangularSolver_DSCC.solveTranColumn(G, x, xi, w, pinv, X_nz_count, col);
                }
            } else {
    ...
    private static int solveTranColumn(DMatrixSparseCSC G, double[] x, int[] xi, int[] w, @Nullable int[] pinv, int x_nz_count, int col) {
    ...
    public static void solve(DMatrixSparseCSC G, boolean lower, DMatrixSparseCSC B, DMatrixSparseCSC X, @Nullable int[] pinv, @Nullable DGrowArray g_x, @Nullable IGrowArray g_xi, @Nullable IGrowArray g_w) {
        double[] x = UtilEjml.adjust(g_x, G.numRows);
        if (g_xi == null) {
            g_xi = new IGrowArray();
        }
        ...
        X.indicesSorted = false;
        for (int colB = 0; colB < B.numCols; ++colB) {
            int top = TriangularSolver_DSCC.solveColB(G, lower, B, colB, x, pinv, g_xi, w);
    ...
    public static int solveColB(DMatrixSparseCSC G, boolean lower, DMatrixSparseCSC B, int colB, double[] x, @Nullable int[] pinv, @Nullable IGrowArray g_xi, int[] w) {
        int top;
        int X_rows = G.numCols;
        int[] xi = UtilEjml.adjust(g_xi, X_rows);
        for (int p = top = TriangularSolver_DSCC.searchNzRowsInX(G, B, colB, pinv, xi, w); p < X_rows; ++p) {
            x[xi[p]] = 0.0;
        }
    ...
    public static int searchNzRowsInX(DMatrixSparseCSC G, DMatrixSparseCSC B, int colB, @Nullable int[] pinv, int[] xi, int[] w) {
        int i;
        int X_rows = G.numCols;
        if (xi.length < X_rows) {
        ...
        for (i = idx0; i < idx1; ++i) {
            ...
            top = TriangularSolver_DSCC.searchNzRowsInX_DFS(rowB, G, top, pinv, xi, w);
        }
