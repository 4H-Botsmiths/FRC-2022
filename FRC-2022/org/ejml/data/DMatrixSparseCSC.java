...
implements DMatrixSparse {
    ...
    public void shrinkArrays() {
        if (this.nz_length < this.nz_values.length) {
            double[] tmp_values = new double[this.nz_length];
    ...
    public Iterator<DMatrixSparse.CoordinateRealValue> createCoordinateIterator() {
        return new Iterator<DMatrixSparse.CoordinateRealValue>(){
            ...
            private void incrementColumn() {
                while (this.column + 1 <= DMatrixSparseCSC.this.numCols && this.nz_index >= DMatrixSparseCSC.this.col_idx[this.column + 1]) {
                    ++this.column;
                }
            }
        };
