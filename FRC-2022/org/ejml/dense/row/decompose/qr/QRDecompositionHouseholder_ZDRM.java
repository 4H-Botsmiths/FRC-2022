...
implements QRDecomposition<ZMatrixRMaj> {
    ...
    public void setExpectedMaxSize(int numRows, int numCols) {
        ...
        if (this.QR == null) {
            this.QR = new ZMatrixRMaj(numRows, numCols);
    ...
    public ZMatrixRMaj getQ(@Nullable ZMatrixRMaj Q, boolean compact) {
        ...
        for (int j = this.minLength - 1; j >= 0; --j) {
            QrHelperFunctions_ZDRM.extractHouseholderColumn(this.QR, j, this.numRows, j, this.u, 0);
            QrHelperFunctions_ZDRM.rank1UpdateMultR(Q, this.u, 0, this.gammas[j], j, j, this.numRows, this.v);
        }
        return Q;
    }
    ...
    protected void householder(int j) {
        double max = QrHelperFunctions_ZDRM.extractColumnAndMax(this.QR, j, this.numRows, j, this.u, 0);
        if (max <= 0.0) {
            this.gammas[j] = 0.0;
            this.error = true;
        } else {
            double gamma;
            this.gammas[j] = gamma = QrHelperFunctions_ZDRM.computeTauGammaAndDivide(j, this.numRows, this.u, max, this.tau);
            double real_u_0 = this.u[j * 2] + this.tau.real;
            double imag_u_0 = this.u[j * 2 + 1] + this.tau.imaginary;
            QrHelperFunctions_ZDRM.divideElements(j + 1, this.numRows, this.u, 0, real_u_0, imag_u_0);
            ...
            this.u[j * 2] = 1.0;
            this.u[j * 2 + 1] = 0.0;
            QrHelperFunctions_ZDRM.rank1UpdateMultR(this.QR, this.u, 0, gamma, j + 1, j, this.numRows, this.v);
            if (j < this.numCols) {
