...
public class ImplCommonOpsWithSemiRing_FSCC {
    public static void add(float alpha, FMatrixSparseCSC A, float beta, FMatrixSparseCSC B, FMatrixSparseCSC C, FSemiRing semiRing, @Nullable Mask mask, @Nullable IGrowArray gw, @Nullable FGrowArray gx) {
        ...
        for (int col = 0; col < A.numCols; ++col) {
            ...
            ImplMultiplicationWithSemiRing_FSCC.multAddColA(A, col, alpha, C, col + 1, semiRing, mask, x, w);
            ImplMultiplicationWithSemiRing_FSCC.multAddColA(B, col, beta, C, col + 1, semiRing, mask, x, w);
    ...
    public static void addColAppend(FMatrixSparseCSC A, int colA, FMatrixSparseCSC B, int colB, FMatrixSparseCSC C, FSemiRing semiRing, @Nullable IGrowArray gw) {
        int row;
        int i;
    ...
    public static void elementMult(FMatrixSparseCSC A, FMatrixSparseCSC B, FMatrixSparseCSC C, FSemiRing semiRing, @Nullable Mask mask, @Nullable IGrowArray gw, @Nullable FGrowArray gx) {
        float[] x = UtilEjml.adjust(gx, A.numRows);
        int[] w = UtilEjml.adjust(gw, A.numRows);
        Arrays.fill(w, 0, A.numRows, -1);
