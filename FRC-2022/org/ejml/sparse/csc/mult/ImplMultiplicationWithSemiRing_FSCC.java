...
public class ImplMultiplicationWithSemiRing_FSCC {
    public static void mult(FMatrixSparseCSC A, FMatrixSparseCSC B, FMatrixSparseCSC C, FSemiRing semiRing, @Nullable Mask mask, @Nullable IGrowArray gw, @Nullable FGrowArray gx) {
        ...
        for (int bj = 1; bj <= B.numCols; ++bj) {
            ...
            for (int bi = idx0; bi < idx1; ++bi) {
                ...
                ImplMultiplicationWithSemiRing_FSCC.multAddColA(A, rowB, valB, C, colB + 1, semiRing, mask, x, w);
            }
    ...
    public static void multAddColA(FMatrixSparseCSC A, int colA, float alpha, FMatrixSparseCSC C, int mark, FSemiRing semiRing, @Nullable Mask mask, float[] x, int[] w) {
    ...
    public static void mult(FMatrixSparseCSC A, FMatrixRMaj B, FMatrixRMaj C, FSemiRing semiRing) {
        C.fill(semiRing.add.id);
        ImplMultiplicationWithSemiRing_FSCC.multAdd(A, B, C, semiRing);
    }

    public static void multAdd(FMatrixSparseCSC A, FMatrixRMaj B, FMatrixRMaj C, FSemiRing semiRing) {
    ...
    public static void multTransA(FMatrixSparseCSC A, FMatrixRMaj B, FMatrixRMaj C, FSemiRing semiRing) {
    ...
    public static void multAddTransA(FMatrixSparseCSC A, FMatrixRMaj B, FMatrixRMaj C, FSemiRing semiRing) {
        for (int j = 0; j < B.numCols; ++j) {
            for (int i = 0; i < A.numCols; ++i) {
                int idx0 = A.col_idx[i];
                int idx1 = A.col_idx[i + 1];
    ...
    public static void multTransB(FMatrixSparseCSC A, FMatrixRMaj B, FMatrixRMaj C, FSemiRing semiRing) {
        C.zero();
        ImplMultiplicationWithSemiRing_FSCC.multAddTransB(A, B, C, semiRing);
    }

    public static void multAddTransB(FMatrixSparseCSC A, FMatrixRMaj B, FMatrixRMaj C, FSemiRing semiRing) {
        for (int k = 0; k < A.numCols; ++k) {
            int idx0 = A.col_idx[k];
            int idx1 = A.col_idx[k + 1];
            for (int indexA = idx0; indexA < idx1; ++indexA) {
    ...
    public static void multTransAB(FMatrixSparseCSC A, FMatrixRMaj B, FMatrixRMaj C, FSemiRing semiRing) {
        C.zero();
