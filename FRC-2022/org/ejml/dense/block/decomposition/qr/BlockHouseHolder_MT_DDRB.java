...
public class BlockHouseHolder_MT_DDRB {
    public static boolean decomposeQR_block_col(int blockLength, DSubmatrixD1 Y, double[] gamma) {
        ...
        for (int i = 0; i < min; ++i) {
            if (!BlockHouseHolder_MT_DDRB.computeHouseHolderCol(blockLength, Y, gamma, i)) {
                return false;
            }
            BlockHouseHolder_MT_DDRB.rank1UpdateMultR_Col(blockLength, Y, i, gamma[Y.col0 + i]);
        }
        return true;
    }
    ...
    public static boolean computeHouseHolderCol(int blockLength, DSubmatrixD1 Y, double[] gamma, int i) {
        ...
        double tau = BlockHouseHolder_MT_DDRB.computeTauAndDivideCol(blockLength, Y, i, max);
        double u_0 = Y.get(i, i) + tau;
        BlockHouseHolder_MT_DDRB.divideElementsCol(blockLength, Y, i, u_0);
    ...
    public static boolean computeHouseHolderRow(int blockLength, DSubmatrixD1 Y, double[] gamma, int i) {
        ...
        double tau = BlockHouseHolder_MT_DDRB.computeTauAndDivideRow(blockLength, Y, i, i + 1, max);
    ...
    public static void rank1UpdateMultR_Col(int blockLength, DSubmatrixD1 A, int col, double gamma) {
        ...
        EjmlConcurrency.loopFor(col + 1, width, j -> {
            double total = BlockHouseHolder_MT_DDRB.innerProdCol(blockLength, A, col, width, j, width);
            total *= gamma;
    ...
    public static void rank1UpdateMultR_TopRow(int blockLength, DSubmatrixD1 A, int col, double gamma) {
        ...
        for (int colStartJ = A.col0 + blockLength; colStartJ < A.col1; colStartJ += blockLength) {
            ...
            for (int j = 0; j < widthJ; ++j) {
                double total = BlockHouseHolder_MT_DDRB.innerProdCol(blockLength, A, col, widthCol, colStartJ - A.col0 + j, widthJ) * gamma;
                int i = A.row0;
                int height = Math.min(blockLength, A.row1 - i);
                int indexU = i * ((DMatrixD1)A.original).numCols + height * A.col0 + col;
    ...
    public static void rank1UpdateMultL_Row(int blockLength, DSubmatrixD1 A, int row, int colStart, double gamma) {
        ...
        for (int i = row + 1; i < height; ++i) {
            double total = BlockHouseHolder_MT_DDRB.innerProdRow(blockLength, A, row, A, i, zeroOffset);
            total *= gamma;
    ...
    public static void rank1UpdateMultL_LeftCol(int blockLength, DSubmatrixD1 A, int row, double gamma, int zeroOffset) {
        ...
        for (int blockStart = A.row0 + blockLength; blockStart < A.row1; blockStart += blockLength) {
            ...
            for (int i = 0; i < heightA; ++i) {
                double total = BlockHouseHolder_MT_DDRB.innerProdRow(blockLength, A, row, A, i + (blockStart - A.row0), zeroOffset);
    ...
    public static void computeW_Column(int blockLength, DSubmatrixD1 Y, DSubmatrixD1 W, @Nullable GrowArray<DGrowArray> workspace, double[] beta, int betaIndex) {
        ...
        BlockHouseHolder_MT_DDRB.initializeW(blockLength, W, Y, widthB, beta[betaIndex]);
        int min = Math.min(widthB, W.row1 - W.row0);
        double[] temp = workspace.grow().reshape((int)(Y.col1 - Y.col0)).data;
        for (int j = 1; j < min; ++j) {
            BlockHouseHolder_MT_DDRB.computeY_t_V(blockLength, Y, j, temp);
            BlockHouseHolder_MT_DDRB.computeZ(blockLength, Y, W, j, temp, beta[betaIndex + j]);
        }
    }
    ...
    public static void computeY_t_V(int blockLength, DSubmatrixD1 Y, int col, double[] temp) {
        ...
        EjmlConcurrency.loopFor(0, col, j -> {
            temp[j] = BlockHouseHolder_MT_DDRB.innerProdCol(blockLength, Y, col, widthB, j, widthB);
        });
    }
        ...
        EjmlConcurrency.loopFor(Y.row0, Y.row1, blockLength, i -> {
            ...
            for (int j = B.col0; j < B.col1; j += blockLength) {
                ...
                for (int k = Y.col0; k < Y.col1; k += blockLength) {
                    ...
                    if (i == Y.row0) {
                        BlockHouseHolder_MT_DDRB.multBlockAdd_zerosone(((DMatrixD1)Y.original).data, ((DMatrixD1)B.original).data, ((DMatrixD1)C.original).data, indexY, indexB, indexC, heightY, widthY, widthB);
                        continue;
                    }
    ...
    public static void multTransA_vecCol(int blockLength, DSubmatrixD1 A, DSubmatrixD1 B, DSubmatrixD1 C) {
        ...
        EjmlConcurrency.loopFor(B.col0, B.col1, blockLength, j -> {
            ...
            for (int k = A.row0; k < A.row1; k += blockLength) {
                ...
                if (k == A.row0) {
                    BlockHouseHolder_MT_DDRB.multTransABlockSet_lowerTriag(((DMatrixD1)A.original).data, ((DMatrixD1)B.original).data, ((DMatrixD1)C.original).data, indexA, indexB, indexC, heightA, widthA, widthB);
                    continue;
                }
