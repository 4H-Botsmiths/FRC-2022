...
public class BlockHouseHolder_FDRB {
    public static boolean decomposeQR_block_col(int blockLength, FSubmatrixD1 Y, float[] gamma) {
        ...
        for (int i = 0; i < min; ++i) {
            if (!BlockHouseHolder_FDRB.computeHouseHolderCol(blockLength, Y, gamma, i)) {
                return false;
            }
            BlockHouseHolder_FDRB.rank1UpdateMultR_Col(blockLength, Y, i, gamma[Y.col0 + i]);
        }
        return true;
    }
    ...
    public static boolean computeHouseHolderCol(int blockLength, FSubmatrixD1 Y, float[] gamma, int i) {
        ...
        float tau = BlockHouseHolder_FDRB.computeTauAndDivideCol(blockLength, Y, i, max);
        float u_0 = Y.get(i, i) + tau;
        BlockHouseHolder_FDRB.divideElementsCol(blockLength, Y, i, u_0);
    ...
    public static boolean computeHouseHolderRow(int blockLength, FSubmatrixD1 Y, float[] gamma, int i) {
        ...
        float tau = BlockHouseHolder_FDRB.computeTauAndDivideRow(blockLength, Y, i, i + 1, max);
    ...
    public static void rank1UpdateMultR_Col(int blockLength, FSubmatrixD1 A, int col, float gamma) {
        ...
        for (int j = col + 1; j < width; ++j) {
            float total = BlockHouseHolder_FDRB.innerProdCol(blockLength, A, col, width, j, width);
            total *= gamma;
    ...
    public static void rank1UpdateMultR_TopRow(int blockLength, FSubmatrixD1 A, int col, float gamma) {
        ...
        for (int colStartJ = A.col0 + blockLength; colStartJ < A.col1; colStartJ += blockLength) {
            ...
            for (int j = 0; j < widthJ; ++j) {
                float total = BlockHouseHolder_FDRB.innerProdCol(blockLength, A, col, widthCol, colStartJ - A.col0 + j, widthJ) * gamma;
                int i = A.row0;
                int height = Math.min(blockLength, A.row1 - i);
                int indexU = i * ((FMatrixD1)A.original).numCols + height * A.col0 + col;
    ...
    public static void rank1UpdateMultL_Row(int blockLength, FSubmatrixD1 A, int row, int colStart, float gamma) {
        ...
        for (int i = row + 1; i < height; ++i) {
            float total = BlockHouseHolder_FDRB.innerProdRow(blockLength, A, row, A, i, zeroOffset);
            total *= gamma;
    ...
    public static void rank1UpdateMultL_LeftCol(int blockLength, FSubmatrixD1 A, int row, float gamma, int zeroOffset) {
        ...
        for (int blockStart = A.row0 + blockLength; blockStart < A.row1; blockStart += blockLength) {
            ...
            for (int i = 0; i < heightA; ++i) {
                float total = BlockHouseHolder_FDRB.innerProdRow(blockLength, A, row, A, i + (blockStart - A.row0), zeroOffset);
    ...
    public static void computeW_Column(int blockLength, FSubmatrixD1 Y, FSubmatrixD1 W, @Nullable GrowArray<FGrowArray> workspace, float[] beta, int betaIndex) {
        ...
        BlockHouseHolder_FDRB.initializeW(blockLength, W, Y, widthB, beta[betaIndex]);
        int min = Math.min(widthB, W.row1 - W.row0);
        float[] temp = workspace.grow().reshape((int)(Y.col1 - Y.col0)).data;
        for (int j = 1; j < min; ++j) {
            BlockHouseHolder_FDRB.computeY_t_V(blockLength, Y, j, temp);
            BlockHouseHolder_FDRB.computeZ(blockLength, Y, W, j, temp, beta[betaIndex + j]);
        }
    }
    ...
    public static void computeY_t_V(int blockLength, FSubmatrixD1 Y, int col, float[] temp) {
        ...
        for (int j = 0; j < col; ++j) {
            temp[j] = BlockHouseHolder_FDRB.innerProdCol(blockLength, Y, col, widthB, j, widthB);
        }
    }
        ...
        for (int i = Y.row0; i < Y.row1; i += blockLength) {
            ...
            for (int j = B.col0; j < B.col1; j += blockLength) {
                ...
                for (int k = Y.col0; k < Y.col1; k += blockLength) {
                    ...
                    if (i == Y.row0) {
                        BlockHouseHolder_FDRB.multBlockAdd_zerosone(((FMatrixD1)Y.original).data, ((FMatrixD1)B.original).data, ((FMatrixD1)C.original).data, indexY, indexB, indexC, heightY, widthY, widthB);
                        continue;
                    }
    ...
    public static void multTransA_vecCol(int blockLength, FSubmatrixD1 A, FSubmatrixD1 B, FSubmatrixD1 C) {
        ...
        for (int j = B.col0; j < B.col1; j += blockLength) {
            ...
            for (int k = A.row0; k < A.row1; k += blockLength) {
                ...
                if (k == A.row0) {
                    BlockHouseHolder_FDRB.multTransABlockSet_lowerTriag(((FMatrixD1)A.original).data, ((FMatrixD1)B.original).data, ((FMatrixD1)C.original).data, indexA, indexB, indexC, heightA, widthA, widthB);
                    continue;
                }
