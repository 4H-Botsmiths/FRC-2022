...
implements QRDecomposition<DMatrixRBlock> {
    ...
    public DMatrixRBlock getQ(@Nullable DMatrixRBlock Q, boolean compact) {
        Q = QRDecompositionHouseholder_DDRB.initializeQ(Q, this.dataA.numRows, this.dataA.numCols, this.blockLength, compact);
        this.applyQ(Q, true);
        return Q;
    }
    ...
    public void applyQ(DMatrixRBlock B, boolean isIdentity) {
        ...
        for (int i = start; i >= 0; i -= this.blockLength) {
            ...
            if (!this.saveW) {
                BlockHouseHolder_DDRB.computeW_Column(this.blockLength, this.Y, this.W, this.workspace, this.gammas, this.Y.col0);
            }
            BlockHouseHolder_DDRB.multTransA_vecCol(this.blockLength, this.Y, subB, this.WTA);
            MatrixMult_DDRB.multPlus(this.blockLength, this.W, this.WTA, subB);
        }
    }
    ...
    public void applyQTran(DMatrixRBlock B) {
        ...
        for (int i = 0; i < minDimen; i += this.blockLength) {
            ...
            MatrixMult_DDRB.multTransA(this.blockLength, this.W, subB, this.WTA);
            BlockHouseHolder_DDRB.multAdd_zeros(this.blockLength, this.Y, this.WTA, subB);
        }
    }
    ...
    protected void updateA(DSubmatrixD1 A) {
        ...
        if (A.col1 > A.col0) {
            BlockHouseHolder_DDRB.computeW_Column(this.blockLength, this.Y, this.W, this.workspace, this.gammas, this.Y.col0);
            MatrixMult_DDRB.multTransA(this.blockLength, this.W, A, this.WTA);
            BlockHouseHolder_DDRB.multAdd_zeros(this.blockLength, this.Y, this.WTA, A);
        } else if (this.saveW) {
            BlockHouseHolder_DDRB.computeW_Column(this.blockLength, this.Y, this.W, this.workspace, this.gammas, this.Y.col0);
        }
    }
