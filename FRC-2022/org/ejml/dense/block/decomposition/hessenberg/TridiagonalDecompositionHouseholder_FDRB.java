...
implements TridiagonalSimilarDecomposition_F32<FMatrixRBlock> {
    ...
    public FMatrixRBlock getQ(@Nullable FMatrixRBlock Q, boolean transposed) {
        ...
        for (int i = start; i >= 0; i -= this.A.blockLength) {
            ...
            subQ.row0 = i;
            if (transposed) {
                MatrixMult_FDRB.multTransB(this.A.blockLength, subQ, subU, tmp);
            } else {
                MatrixMult_FDRB.mult(this.A.blockLength, subU, subQ, tmp);
            }
            if (transposed) {
                MatrixMult_FDRB.multPlus(this.A.blockLength, tmp, subW, subQ);
            } else {
                MatrixMult_FDRB.multPlusTransA(this.A.blockLength, subW, tmp, subQ);
            }
    ...
    public boolean decompose(FMatrixRBlock orig) {
        ...
        for (int i = 0; i < N; i += this.A.blockLength) {
            ...
            TridiagonalDecompositionHouseholder_FDRB.multPlusTransA(this.A.blockLength, subU, subV, subA);
            TridiagonalDecompositionHouseholder_FDRB.multPlusTransA(this.A.blockLength, subV, subU, subA);
