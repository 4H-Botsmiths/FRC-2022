...
public class Converters {
    public static Mat vector_Point_to_Mat(List<Point> pts) {
        return Converters.vector_Point_to_Mat(pts, 4);
    }

    public static Mat vector_Point2f_to_Mat(List<Point> pts) {
        return Converters.vector_Point_to_Mat(pts, 5);
    }

    public static Mat vector_Point2d_to_Mat(List<Point> pts) {
        return Converters.vector_Point_to_Mat(pts, 6);
    }
    ...
    public static Mat vector_Point_to_Mat(List<Point> pts, int typeDepth) {
        ...
        block9: {
            block8: {
                int count;
                int n = count = pts != null ? pts.size() : 0;
    ...
    public static Mat vector_Point3i_to_Mat(List<Point3> pts) {
        return Converters.vector_Point3_to_Mat(pts, 4);
    }

    public static Mat vector_Point3f_to_Mat(List<Point3> pts) {
        return Converters.vector_Point3_to_Mat(pts, 5);
    }

    public static Mat vector_Point3d_to_Mat(List<Point3> pts) {
        return Converters.vector_Point3_to_Mat(pts, 6);
    }
    ...
    public static void Mat_to_vector_Point2f(Mat m, List<Point> pts) {
        Converters.Mat_to_vector_Point(m, pts);
    }
    ...
    public static void Mat_to_vector_Point3i(Mat m, List<Point3> pts) {
        Converters.Mat_to_vector_Point3(m, pts);
    }
    ...
    public static Mat vector_Mat_to_Mat(List<Mat> mats) {
        Mat res;
        int count;
        int n = count = mats != null ? mats.size() : 0;
        if (count > 0) {
            res = new Mat(count, 1, CvType.CV_32SC2);
            int[] buff = new int[count * 2];
            for (int i = 0; i < count; ++i) {
                long addr = mats.get((int)i).nativeObj;
                buff[i * 2] = (int)(addr >> 32);
                buff[i * 2 + 1] = (int)(addr & 0xFFFFFFFFFFFFFFFFL);
            }
            res.put(0, 0, buff);
        } else {
            res = new Mat();
    ...
    public static Mat vector_uchar_to_Mat(List<Byte> bs) {
        Mat res;
        int count;
        int n = count = bs != null ? bs.size() : 0;
        if (count > 0) {
            res = new Mat(count, 1, CvType.CV_8UC1);
            byte[] buff = new byte[count];
            for (int i = 0; i < count; ++i) {
                byte b;
                buff[i] = b = bs.get(i).byteValue();
            }
            res.put(0, 0, buff);
        } else {
            res = new Mat();
    ...
    public static Mat vector_int_to_Mat(List<Integer> is) {
        Mat res;
        int count;
        int n = count = is != null ? is.size() : 0;
        if (count > 0) {
            res = new Mat(count, 1, CvType.CV_32SC1);
            int[] buff = new int[count];
            for (int i = 0; i < count; ++i) {
                int v;
                buff[i] = v = is.get(i).intValue();
            }
            res.put(0, 0, buff);
        } else {
            res = new Mat();
    ...
    public static Mat vector_KeyPoint_to_Mat(List<KeyPoint> kps) {
        Mat res;
        int count;
        int n = count = kps != null ? kps.size() : 0;
        if (count > 0) {
    ...
    public static Mat vector_vector_Point_to_Mat(List<MatOfPoint> pts, List<Mat> mats) {
        Mat res;
        int lCount;
        int n = lCount = pts != null ? pts.size() : 0;
        if (lCount > 0) {
            mats.addAll(pts);
            res = Converters.vector_Mat_to_Mat(mats);
        } else {
            res = new Mat();
        }
        return res;
    ...
    public static void Mat_to_vector_vector_Point(Mat m, List<MatOfPoint> pts) {
        ...
        ArrayList<Mat> mats = new ArrayList<Mat>(m.rows());
        Converters.Mat_to_vector_Mat(m, mats);
        for (Mat mi : mats) {
    ...
    public static Mat vector_vector_KeyPoint_to_Mat(List<MatOfKeyPoint> kps, List<Mat> mats) {
        Mat res;
        int lCount;
        int n = lCount = kps != null ? kps.size() : 0;
        if (lCount > 0) {
            mats.addAll(kps);
            res = Converters.vector_Mat_to_Mat(mats);
        } else {
    ...
    public static Mat vector_DMatch_to_Mat(List<DMatch> matches) {
        Mat res;
        int count;
        int n = count = matches != null ? matches.size() : 0;
        if (count > 0) {
            res = new Mat(count, 1, CvType.CV_64FC4);
    ...
    public static Mat vector_vector_DMatch_to_Mat(List<MatOfDMatch> lvdm, List<Mat> mats) {
        Mat res;
        int lCount;
        int n = lCount = lvdm != null ? lvdm.size() : 0;
        if (lCount > 0) {
            mats.addAll(lvdm);
            res = Converters.vector_Mat_to_Mat(mats);
        } else {
    ...
    public static Mat vector_vector_char_to_Mat(List<MatOfByte> lvb, List<Mat> mats) {
        Mat res;
        int lCount;
        int n = lCount = lvb != null ? lvb.size() : 0;
        if (lCount > 0) {
            mats.addAll(lvb);
            res = Converters.vector_Mat_to_Mat(mats);
        } else {
    ...
    public static void Mat_to_vector_vector_char(Mat m, List<List<Byte>> llb) {
        ...
        for (Mat mi : mats) {
            ...
            Converters.Mat_to_vector_char(mi, lb);
            llb.add(lb);
            mi.release();
        }
        mats.clear();
